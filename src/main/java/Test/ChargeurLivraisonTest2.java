...
public class ChargeurLivraisonTest2 {
    ...
    public void TUp2() {
        ...
        PointLivraison dernierPoint = new PointLivraison(1860559399L, 20437, 23772, 900);
        boolean dernierPointPresent = false;
        PointLivraison pointAleatoire = new PointLivraison(25303807L, 19933, 32576, 300);
        boolean pointAleatoirePresent = false;

        for (PointLivraison point : petiteTournee.getListePointLivraisons()) {
            if (point.getId() == (long) dernierPoint.getId() && point.getX() == dernierPoint.getX() && point.getY() == dernierPoint.getY())
                dernierPointPresent = true;
            if (point.getId() == (long) pointAleatoire.getId() && point.getX() == pointAleatoire.getX() && point.getY() == pointAleatoire.getY())
                pointAleatoirePresent = true;
        }
    ...
    public void TUm2() {
        ...
        PointLivraison dernierPoint = new PointLivraison(25495299L, 56584, 58632, 900);
        boolean dernierPointPresent = false;
        PointLivraison pointAleatoire1 = new PointLivraison(25303839L, 61477, 88821, 900, 39600.0, 46800.0);
        boolean pointAleatoire1Present = false;
        PointLivraison pointAleatoire2 = new PointLivraison(194605312L, 48949, 107059, 900, 36000.0, 43200.0);
        boolean pointAleatoire2Present = false;

        for (PointLivraison point : moyenneTournee.getListePointLivraisons()) {
            if (point.getId() == (long) dernierPoint.getId() && point.getX() == dernierPoint.getX() && point.getY() == dernierPoint.getY() && point.getDuree() == dernierPoint.getDuree())
                dernierPointPresent = true;
            if (point.getId() == (long) pointAleatoire1.getId() && point.getX() == pointAleatoire1.getX() && point.getY() == pointAleatoire1.getY() && point.getDebutPlage() == (double) pointAleatoire1.getDebutPlage())
                pointAleatoire1Present = true;
            if (point.getId() == (long) pointAleatoire2.getId() && point.getX() == pointAleatoire2.getX() && point.getY() == pointAleatoire2.getY() && point.getDebutPlage() == (double) pointAleatoire2.getDebutPlage())
                pointAleatoire2Present = true;
        }
    ...
    public void TUg2() {
        ...
        PointLivraison dernierPoint = new PointLivraison(984553654L, 163899, 237643, 300);
        boolean dernierPointPresent = false;
        PointLivraison pointAleatoire1 = new PointLivraison(54803121L, 195365, 250340, 300, 39600.0, 46800.0);
        boolean pointAleatoire1Present = false;
        PointLivraison pointAleatoire2 = new PointLivraison(4242803790L, 233559, 229311, 300, 46800.0, 50400.0);
        boolean pointAleatoire2Present = false;
